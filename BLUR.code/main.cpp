...
void guiViewSynthesis()
{
	...
	sequence[s_index++] = "Teddy";
	sequence[s_index++] = "Cones";
	...
	for (int ii = 0; ii < 6; ii++)
	{
		...
		Mat dest, destdisp, max_disp_l;
		Mat matdiL = imread("img/stereo/" + sequence[ii] + "/disp1.png", 0);
		//Mat matdiR = imread("img/stereo/" + sequence[s_index] + "/disp5.png", 0);
		Mat matimL = imread("img/stereo/" + sequence[ii] + "/view1.png");
		...
		svs.postFilterMethod = StereoViewSynthesis::POST_GAUSSIAN_FILL;
		//svs.inpaintMethod = StereoViewSynthesis::FILL_OCCLUSION_HV;
		//svs.inpaintMethod = StereoViewSynthesis::FILL_OCCLUSION_INPAINT_NS;
		svs.inpaintMethod = StereoViewSynthesis::FILL_OCCLUSION_INPAINT_TELEA;

		svs.warpInterpolationMethod = inter;

		maxFilter(matdiL, max_disp_l, dilation_rad);
		//svs(matimL, max_disp_l, dest, destdisp, alphaPos*0.001, 0, dispAmp);
		svs(matimL, max_disp_l, dest, destdisp, 0.25, 0, dispAmp);
		imwrite("save/" + sequence[ii] + "_2_fukushima.png", dest);

		svs(matimL, max_disp_l, dest, destdisp, 0.5, 0, dispAmp);
		imwrite("save/" + sequence[ii] + "_3_fukushima.png", dest);

		svs(matimL, max_disp_l, dest, destdisp, 0.75, 0, dispAmp);
		imwrite("save/" + sequence[ii] + "_4_fukushima.png", dest);

		svs(matimL, max_disp_l, dest, destdisp, 1.0, 0, dispAmp);
		imwrite("save/" + sequence[ii] + "_5_fukushima.png", dest);

		svs(matimL, max_disp_l, dest, destdisp, 1.25, 0, dispAmp);
		imwrite("save/" + sequence[ii] + "_6_fukushima.png", dest);
	}
	return;
	//
